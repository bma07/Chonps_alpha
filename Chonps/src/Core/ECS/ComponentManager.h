#ifndef CHONPS_COMPONENT_MANAGER_H
#define CHONPS_COMPONENT_MANAGER_H
...
namespace Chonps
{
	...
	public:
		...
		{
			const type_info* compType = &typeid(T);
		...
		void remove(Entity entity, const type_info* compType) // remove a component type with type_info from an entity
		{
			CHONPS_CORE_ASSERT(m_ComponentTypes.find(compType) != m_ComponentTypes.end(), "Component type not found! Componant may not be registered");
		...
		std::unordered_map<const type_info*, std::shared_ptr<BaseComponentArray>> get_component_arrays()
		{
		...
		bool has(Entity entity, const type_info* component) // returns true if the entity has the component type. Note: components entered need to be registered first
		{
		...
		ComponentType type(const type_info* compType) // find the component type with type_info from an entity if the component exists
		{
			CHONPS_CORE_ASSERT(m_ComponentTypes.find(compType) != m_ComponentTypes.end(), "Component type not found! Componant may not be registered");
			return m_ComponentTypes[compType];
		}
	...
	private:
		std::unordered_map<const type_info*, ComponentType> m_ComponentTypes;
		std::unordered_map<const type_info*, std::shared_ptr<BaseComponentArray>> m_ComponentArrays;
